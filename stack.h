...
Stack<T>::Stack(const Stack<T>& rhs) {
  ...
  if (rhs._head == nullptr) {
      ...
      Node<T> *rhsHead = rhs._head;
      Node<T> *temp = rhsHead;
      while (temp->_next != nullptr) {
          current->_next = new Node<T>(temp->_next->_data);
          temp = temp->_next;
...
const Stack<T>& Stack<T>::operator=(const Stack<T>& rhs) {
  ...
  if (this != &rhs){
    ...
    Node<T> *front = rhs._head->_next;  
    Node<T> *head = new Node<T>(rhs._head->_data);
    Node<T> *prev = head;
    while (front != nullptr) {
      prev->_next = new Node<T>(front->_data);
      front = front->_next;
      prev = prev->_next;
    }  
  }
